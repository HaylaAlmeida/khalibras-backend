...
export class EmployeeController {
  ...
  async create(
    ...
    user.role = PermissionKeys.EMPLOYEE
  ...
  async count(
    ...
    return this.userRepository.count({role: PermissionKeys.EMPLOYEE, ...where});
  }
  ...
  async find(
    ...
    return this.userRepository.find({where: {role: PermissionKeys.EMPLOYEE, ...filter?.where}, ...filter});
  }
  ...
  async updateAll(
    @requestBody({
      content: {
        'application/json': {
          schema: getModelSchemaRef(User, {partial: true, exclude: ['accessCode']}),
        },
      },
    })
      user: User,
